...
@ace.on_message(filters.incoming & filters.command("drm", prefixes=prefixes))
@auth_required
async def drm(bot: ace, m: Message):
    ...
    try:
        ...
        # Fixed parameter order for Upload_to_Tg
        UL = Upload_to_Tg(bot=bot, m=m, name=name, file_path=filename,
                          path=path, Thumb=Thumb, show_msg=prog, caption=cc)
        await UL.upload_video()
        print("Done")
        
    except Exception as e:
        await prog.delete(True)
        await m.reply_text(f"**Error**\n\n`{str(e)}`\n\nOr May be Video not Available in {Q}p")
    finally:
        if os.path.exists(tPath):
            shutil.rmtree(tPath)
        if os.path.exists(path):
            shutil.rmtree(path)
        await m.reply_text("✅ **DRM Download Complete!**")
